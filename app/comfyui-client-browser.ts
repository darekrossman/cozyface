...
async function queuePrompt(prompt: Prompt, clientId: string): Promise<QueuePromptResponse> {
...
function getImages(ws: WebSocket, prompt: Prompt, clientId: string): Promise<Record<string, Blob[]>> {
    ...
    try {
      ...
      ws.onmessage = async (event: MessageEvent) => {
        if (typeof event.data === 'string') {
          ...
          if (currentNode && prompt[currentNode]?._meta?.title === 'SaveImageWebsocket') {
