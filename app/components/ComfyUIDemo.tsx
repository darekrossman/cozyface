...
export default function ComfyUIDemo() {
  ...
  const [promptText, setPromptText] = useState('Portrait of a cyberpunk cyborg in a dystopian alley');  
  ...
  useEffect(() => {
    const initWebSocket = () => {
      ...
      const ws = new WebSocket(`ws://${serverAddress}/ws?clientId=${clientIdRef.current}`);
  ...
  const generateImage = useCallback(async () => {
    ...
    try {
      const imageResults = await getImages(wsRef.current, prompt, clientIdRef.current);
